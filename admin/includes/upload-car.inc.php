        ...
        if($fileError === 0){
            ...
            if($fileSize < 50000){
                ...
                if(empty($_POST['autonaam']||$_POST['motor']||$_POST['kmstand']||$_POST['bouwjaar']||$_POST['brandstof']||$_POST['transmissie']||$_POST['prijs']||$_POST['verbruik']||$_POST['uitstoot']||$_POST['pk']||$_POST['kw'])){
                    header('location: ../car-add.php?=empty');
                    exit();
                }else{
                    ...
                    if(!mysqli_stmt_prepare($stmt,$sql)){
                        ...
                        $sql= "INSERT INTO autos (autonaam,motor,kmstand,bouwjaar,brandstof,transmissie,prijs,thumbnail,verbruik,uitstoot,vermogenKW,vermogenPK) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
                        if(!mysqli_stmt_prepare($stmt,$sql)){
                            echo "Sql statement failed";
                        }
                        else{
